...
#pragma warning disable 1591 // disable the xmlComments warning
        ...
        protected DatabaseMgr _dbMgr = null;
        protected LoggingMgr _logMgr = null;
        ...
        /// </summary>
        /// <param name="dbMgr">A data access manager object</param>
        ...
        public TaskProcess(DatabaseMgr dbMgr
            , LoggingMgr LogMgr
            , string taskId
            , int taskQueueCode
            , string parameters
                    ...
                    , _threadPoolLabel, _taskQueueCode)))
            {
                try
                {
                          ...
                          || _processStatus == ProcessStatusEnum.Working)
                    {
                        ...
                        _logMgr.Trace("Working", TraceLevels.Level5);
                    }
                }
                catch
                {
                ...
                _logMgr.Trace("Finished", TraceLevels.Level5);
            }
        }
        ...
        public void Stop()
        {
            using (LoggingContext lc = new LoggingContext(string.Format("ThreadPool: {0}; TaskProcess QueueCode: {1}"
                    , _threadPoolLabel, _taskQueueCode)))
            {
                _logMgr.Trace("Stopping", TraceLevels.Level5);
        ...
        public void Pause()
        {
            using (LoggingContext lc = new LoggingContext(string.Format("ThreadPool: {0}; TaskProcess QueueCode: {1}"
                    , _threadPoolLabel, _taskQueueCode)))
            {
                _logMgr.Trace("Pausing", TraceLevels.Level5);
        ...
        public void Resume()
        {
            using (LoggingContext lc = new LoggingContext(string.Format("ThreadPool: {0}; TaskProcess QueueCode: {1}"
                    , _threadPoolLabel, _taskQueueCode)))
            {
                _logMgr.Trace("Resuming", TraceLevels.Level5);
                _processStatus = ProcessStatusEnum.Working;
